...
var DynamicInputModel = (function (_super) {
    ...
    function DynamicInputModel(config, cls) {
        ...
        _this.files = null;
        _this.listId = null;
        _this.type = exports.DYNAMIC_FORM_CONTROL_TYPE_INPUT;
        _this.accept = utils_1.getValue(config, "accept", null);
        _this.inputType = utils_1.getValue(config, "inputType", exports.DYNAMIC_FORM_CONTROL_INPUT_TYPE_TEXT);
        _this.list = utils_1.getValue(config, "list", null);
        _this.max = utils_1.getValue(config, "max", null);
        _this.min = utils_1.getValue(config, "min", null);
        _this.multiple = utils_1.getValue(config, "multiple", null);
        _this.pattern = utils_1.getValue(config, "pattern", null);
        _this.step = utils_1.getValue(config, "step", null);
        if (_this.list) {
